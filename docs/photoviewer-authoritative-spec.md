...
| Settings | `.cache/settings.json` | Browser app settings/key bindings。WPFは`thumbnailStatusBorders` subtreeをBrowserとread-writeし、dirty preferenceだけをlatest diskへmerge |
| Albums | `.cache/albums.json` | version 1、opaque Album/member id、document/Album revision。Browser/WPF read-write、WinForms非参加 |
...
Favorite/settings/recent/seen/Album writeはtarget別create-new lock、bounded retry、30秒stale recovery、lock内latest-disk再読、temp+atomic replaceを使う。structured lock ownerのPIDがliveならageだけでstealしない。malformed/future documentは空で上書きせず、409またはrecoverable refusalにする。unknown fieldと他writerの無関係entryを保持する。Albumの旧versionなし文書は`albums: []`以外のv1固有fieldを持たない曖昧性のないemptyだけをv1へmigrationし、それ以外は拒否する。
...
- filtered/sorted Modal orderを縦に示す左側virtualized Filmstrip。manual UI表示中は画像用center areaから分離した左rail、manual UI非表示中は左端hoverの前面railとして出し、離脱時に畳んで画像geometryを動かさない。current imageへ追従し、thumbnail clickで直接移動
...
Filmstripはviewport + 上下8 item overscanだけをrealizeし、100,000 logical imagesでも全thumbnail DOMを作らない。sparse orderはvisible/overscan rangeと交差するsearch pageだけを取得する。active image変更時はstripを中央へscrollするが、通常navigationでfocusを強制移動しない。toolbarとstrip内Hide、configurable既定`T`で開閉し、`pvu_view.modalFilmstripOpen`へ保存する。Filmstrip内ArrowUp/DownとLeft/RightはModal previous/nextを1回だけ実行する。strip上のpointer/click/wheelをcenter chrome toggle、edge click、pan、swipe、wheel zoomへ漏らさない。
...
SettingsはBehavior、editable key bindings、Runtime/Version diagnosticsを持つ。panel内の1つのbounded scroll owner、backdrop click close、initial focus、Tab trap、Escape、focus return、loading/error live regionを備える。key conflict、invalid chord、reserved chordをinline表示し、Save成功時だけactive mapへ反映する。enabled buttonはpointer hoverとkeyboard focusで明瞭に変化し、disabled stateと混同しない。
...
Modal image areaの右clickは製品context menuを開き、Favorite +1/-1、Original/Enhanced、guarded external open、Album add、Filmstrip、metadata、zoom reset、確認付きsource Recycleを提供する。menuはEscape/outside actionで閉じ、背面shortcutを隔離する。Explorer/path copyと画像bitmap/file copyはまだ含まない。Ctrl+CはPrompt等のtext copy以外、画像bitmap/file copyとして定義されていないため、clipboard部分はSection 14のPENDINGである。
...
| Add selection/current image to Album | B (collision-aware free fallback) |
...
- Filmstripはvertical listbox/option、current 1件の`aria-current`/`aria-selected`、logical全件中の`aria-posinset`/`aria-setsize`を持つ。toolbar toggleは`aria-expanded`、`aria-controls`、`aria-keyshortcuts`を公開し、strip内Hideは`aria-controls`と`aria-keyshortcuts`を公開する。
...
Headerはsidebar、logo/landing、Refresh、Search、count、Settings/right preview、window controls。Landingにもminimize、maximize/restore、closeを同じnative caption actionとして置く。SidebarはFavorite、manual date、Folders、sort、Grid/List、style、aspect、zoomを持ち、Settingsをscroll contentに埋めず下端へ固定する。Quick Searchとrelative date presetを置かない。
...
Grid/List切替、Standard/Compact/Poster、Original/1:1/2:3を持つ。現行zoomは20〜600、20px step、reset 200。600 endpointはavailable widthに関係なく正確な1列へ固定する。Ctrl+wheelはactive Grid surfaceとそのscrollbar由来を同じgallery gestureとして扱い、入力control、overlay、Landing、非gallery sourceでは拒否する。Ctrl `+/-/0`も同じmodelを使う。middle-clickはGrid/List galleryだけでauto-scrollを開始し、再click/Escape/capture lossで終了する。zoom前にvisible selectionを優先したcanonical full path+viewport offsetをcaptureし、selectionなしではviewport centerを使う。zoom、Sidebar、right panel、window resize、DPI change後に同じpath/offsetをclamp復元し、sidebar/header/font/right panel/List rowを拡縮しない。List modeではzoomを拒否してrecycling virtualizationを維持する。

Grid/List thumbnailはsmall endpointでも過度に細長くせず、neutral backgroundとbounded uniform card geometryを使う。各itemはdirect Favorite +1/-1 actionを持ち、virtualizationやselectionを壊さず同じshared Favorite writerへ送る。
...
filtered/sorted full orderを使い、Left/Right/button/left-right 28% edge zoneでnavigationし、端でwrapする。center 44%、single click manual chrome、double click metadata sidebar、black backdrop close。image areaはwindow全体を使い、画像を`Uniform` fitしてtoolbar/rail以外の利用可能領域へ最大表示する。zoom 0.25〜10、wheel/key、pan、flip、swipe、650ms feedback。

manual chrome表示は操作停止でも出たままとする。imageまたはempty center clickでhide/showし、manual非表示ではcursorも隠し、pointer/keyで約900msだけ一時表示して戻す。非表示状態はnavigation、Delete隣接移動、Original/Enhanced切替後も維持する。manual Filmstripは左の専用vertical rail、非表示中の左端hover Filmstripは画像geometryを変えない前面railとし、`T`/toolbarで開閉してViewerStateへ保存する。倍率はModal root上端に1つだけ低contrast表示する。Modal top buttonへfocus中でもArrow/Delete/T/H/F等を処理し、Enter/Spaceはbutton native activation、TextBox等は隔離する。Prompt chip等のbutton Enter/Spaceもnative clickを1回だけ実行し、gallery/modal openへ漏らさない。

Prompt/Negative/Settings tabs、metadata copy、Prompt chip→Searchを持つ。Prompt表示は`((...))`、`[...]`、`{...}`のemphasis-only wrapperを除き、Browser同様のplain tagとして示す。既存succeeded managed outputがあればModalはEnhancedを既定表示し、Original/Enhanced controlとEで切り替える。現在表示中assetの実体容量をbytes / 1024²、小数2桁、spaceなしのexact `0.00MB`形式で表示し、toggleで即更新する。Enhancedがmissing/stale/invalid signature/ownership外ならOriginal容量とrecoverable statusへfallbackする。explicit AI x2だけがBrowser loopback APIへcreateし、WPF自身はworker/job storeを所有しない。
...
App SettingsはUnseen dots、Confirm before delete、editable key bindings、thumbnail status borders、安全なAbout/Diagnosticsを1つのbounded scroll ownerに収め、backdrop click/Escape/close buttonで閉じる。Favorite枠は独立ON/OFF + `#RRGGBB`で既定黄色、AI高画質化済み枠は独立ON/OFF + `#RRGGBB`で既定cyan `#38bdf8`。旧`rainbow`はread時にcyanへ非破壊migrationし、rainbow UIを再表示しない。Browser / WPFはFavorite/Enhancedのdirty preference単位だけをlock内latest diskへmergeし、もう一方の同時更新を古いdraftで戻さない。Browserは空patchを拒否する。shared `.cache/settings.json`の未知nested keyとextension fieldを保持し、malformed/future/busy書込を非破壊でRetryへ戻す。invalid chordはaction単位fallback、active-context conflict時は曖昧mapを採用しない。すべてのenabled button styleはhover/focus changeを持ち、ToolTipは空白だけを表示せずdark surfaceで説明を示す。
...
Modalの右clickはproduct image context menuを開き、Favorite +1/-1、Original/Enhanced、guarded external open、Show in folder、Album add、Filmstrip、確認付きsource Recycleを提供する。Clipboard操作はsafe diagnosticsとPrompt/Negative/PNG metadata textまでで、画像file/bitmap copyではない。headerのAlbum controlは有効でbasic libraryを開く。Browser/WPFは同じAlbum v1 store/operationを使い、create/rename/delete/pin/cover/recent/add/remove、current/outside/missing表示、current memberのAlbum順navigation、別操作のmembership remove/source Recycleを提供する。Recycle成功後cleanupが失敗した場合はmissing memberとpending statusを保持し、latest documentへretryする。WPF v1のoutside memberは明示unavailableで、任意absolute pathを黙って開かない。context action surfaceは実装中、Ctrl+C画像copyはSection 14のPENDINGである。
...
- Favorite/Enhanced thumbnail枠はshared `thumbnailStatusBorders` schemaを使う。Favorite既定黄色inner、Enhanced既定solid cyan outerを独立ON/OFF/単色設定でき、legacy `rainbow`はcyanへmigrationする。保存はdirty preference単位のlatest-disk mergeとする。Enhancement job lifecycleやsource/output ownershipとは別設定である。
...
Modal filmstripは100,000 logical itemsでもviewport + overscanだけをDOMへ出すunit gateを持つ。production E2Eは120-item fixtureで遠方index click、Arrow往復、Delete隣接移動が二重発火しないこと、current追従、chrome hide、close/reopenとlocalStorage保存を確認する。`9d8acb0`時点のhistorical E2Eは当時の3秒auto-hideを確認したが、現行manual visible / 900ms transient / hidden hover overlayを旧挙動へ読み替えない。2026-07-20の`127.0.0.1:31337` E2Eは当時の下端overlay geometryを確認したhistorical evidenceであり、current left-vertical review deltaのruntime証拠ではない。left-vertical deltaはport 3001+のfinal isolated runtimeが**PENDING FINAL GATE**である。reload hydrationは`ImageContext`のunmount/remount unit gateで確認する。この120-item E2Eを製品件数上限と解釈しない。
...
全routeはloopback trust boundary内だけで使う。`package.json`の`dev`/`start`も`127.0.0.1`へ明示bindする。mutating routeはloopback Hostとsame-origin/local Origin/Fetch Metadataをshared guardで検証し、cross-siteまたは非loopback requestを状態変更前に拒否する。`path`や`dir`を受け取るrouteは、documented legacy例外を除きcurrent `indexToken` snapshot membership、active root、canonical regular fileを検証する。error responseへsecret/envを含めない。明示loopback scriptsとmutating-route guardはcurrent worktreeにあるが、focused/full testsとisolated runtimeが完了するまではreview hardeningの`in progress`である。
...
Public repository/distributionは別gateである。`LICENSE`方針が未決で、full
Codex Security scanも外部相談禁止の本laneでは実行していないため、public
readinessは**NO-GO**とする。private/local運用とdeployment禁止を維持し、focused
security testsをfull scanや公開許可へ読み替えない。

## 14. 採用済み要件と実装確認待ち

2026-07-20にlocal `refs/heads/main`はWPF gallery zoom/geometry anchor実装 `e371b482af44e0428d9fe0d5217b236801f29cff`、shared-state latency `5ae1e00`、表示中asset open/容量表示 `a1d83c8`、WPF focus assertion `dbad550`、Modal button Enter isolation `452ac02`、Browser shared-writer FIFO fairness `0802f95`を含む後続main変更まで採用された。独立review hardeningは`f64d98d`上の未commit worktree deltaで、Browser unit 69 files / 635 tests、typecheck/lint/build、WPF Release/focused verifierまでgreenだが、final focused/normal-scale full、runtime/provenance、public-guard tests、final SHAは**PENDING FINAL GATE**である。20k/100k catalog-scaleとaggregate check-count追跡はユーザーが今回gateから明示除外した。下表のimplementedは完了済みsourceと採用gateへ限定する。
...
| Browser Modal virtualized Filmstrip | current追従、direct click、bounded virtualization、T/toolbar開閉、保存、Arrow navigation。完了済み基盤に対しcurrent worktreeでleft vertical railへ変更 | WPFもleft vertical railと同じUI状態を実装 | baseは**implemented**、vertical review deltaは**PENDING FINAL GATE**。focused Browser/WPF green、final normal-scale full/runtime待ち |
...
| Favorite / AI高画質化thumbnail枠 | Favorite既定yellow inner、Enhanced solid cyan outer。独立ON/OFF、単色、Grid/List、dirty preference latest-disk merge、空patch拒否 | Browserと同じshared schema、solid cyan outer、legacy rainbow→cyan、同じinner/outer優先順 | baseは**implemented**。solid-cyan review deltaはfocused green、final normal-scale full/runtime待ち。unknown/malformed/future/busy保護とlost-update防止を維持 |
| Modal manual/transient UI + Filmstrip parity | manual表示固定、非表示cursor、900ms transient、left-edge vertical overlay、hidden-state維持 | 同じmanual/transient契約、left vertical rail/overlay、button-focus shortcut、ViewerState保存 | state/chrome baseは**implemented**。vertical deltaはfocused green、final normal-scale full/runtime待ち |
...
| Album v1 / collection | operation API、library/picker、guarded source session、current/outside/missing、Album順Gallery/Modal/Filmstrip、remove/Recycle分離、Recycle cleanup retry | shared store/basic library、current-only Album順filter/Modal、outside/missing unavailable、catalog復帰、Recycle cleanup retry | baseは**implemented**。legacy-empty/live-lock/same-state cleanup/unknown-shortcut hardeningはfocused green、final gate待ち |

Review hardening以前の採用gate履歴はBrowser unit 61 files / 521 tests（3 files / 3 tests skip）、typecheck、production build、lint 0 errors、production Playwright 7/7、WPF Search History focused、Browser/WPF 20+20同時writer、Release build 0 warnings / 0 errors、WPF zoom promotion aggregate + reload soak 53/53、reload 24/24がgreen。WPF zoom/anchorはfocusedとexact 100,000 images / 100 foldersも当時のcurrent mainで再実行した。後続shared-state latency descendantは`-SkipStress` aggregate 51/51とfocused latency 6/6がgreen。さらにdisplayed-asset successorの12 owned feature blobsがcheckpoint `8ff1e52`と当時のcurrent mainでbyte-identicalであり、`-IncludeReloadSoak` aggregate 55/55、`AGGREGATE_EXIT=0`、333,899ms、reload soak 24 cycles / 40,322msがgreenだった。これらは変更しないhistorical evidenceで、current review deltaの証明へ横流ししない。ユーザー明示により今回checkpointでのrerunは要求しない。

同じくreview hardening以前の後続delta証拠は上のmilestone件数と混同しない。`9d8acb0`のModal layoutはisolated production port 3001で1/1 PASS、`3654b88`のEnhancement publish/cleanupはfocused 4 files / 23 tests PASS。`a1d83c8`のBrowser displayed-asset focusedは5 files / 71 testsとtypecheck/lint/build、WPF external-open/gallery-enter/modal-enhancement/modal-interaction/status-border focusedとRelease 0/0がgreen。`0802f95`後のBrowser aggregateは63 files / 584 tests PASSだった。2026-07-20 completion audit時点のtreeは63 files / 589 tests PASS（3 files / 3 tests skip）、Next.js 16.2.10 production build、typecheck、scoped lint、Search History focused 33/33、isolated Playwright 1/1がgreen。WPF `-IncludeReloadSoak`は55/55、462,762ms、reload 24/24、cross-runtimeはready barrier後に592〜668msの実write overlapを観測した。fresh real-GPU jobは未実施なので、unit証拠を実machineのend-to-end成功へ読み替えない。

### 14.2 review hardening in progress

| Requirement | Browser worktree | WPF worktree | Remaining acceptance |
| --- | --- | --- | --- |
| vertical Filmstrip / full image Modal | left vertical railとleft-edge transient rail | full-window Uniform image、left vertical rail、28/44/28 edge、single zoom | focused/full normal-scale、isolated Browser/WPF runtime |
| UI controls / Settings | context actions、backdrop、single scroll、hover | landing caption、sidebar-bottom Settings、backdrop/single scroll、hover/dark tooltip、functional Modal controls | Browser/WPF live runtime + final SHA |
| gallery efficiency | existing zoom model | active Grid surface/scrollbar Ctrl+wheel、List row非拡縮、Grid/List middle auto-scroll、direct Favorite、bounded small cards | WPF focused/full normal-scale + runtime |
| safe Album/open hardening | migration/live-lock/cleanup retry/direct non-shell open | migration/cleanup retry/unknown chord reservation | final normal-scale concurrency/full/provenance |
| local API / public readiness | direct argv open adopted、loopback package scriptsとmutating API Origin/Host guardはtests pending | loopback Browser APIだけへdelegate | guard focused/full + isolated runtime pending。`LICENSE`未決、full Codex Security scan未実行のためpublic **NO-GO**、private/localのみ |

### 14.3 引き続き「実装確認待ち」

| Requirement | Browser local main | WPF local main | Canonical status / acceptance |
| --- | --- | --- | --- |
| image context menu completeness | current worktreeにModal action menu。Explorer/path/image copyなし | current worktreeにModal action menu。path/image copyなし | **partial / final gate pending**。runtime、keyboard/focus、clipboard actionsを完了する |
...
| unit/component | `pnpm test:unit` | headless app smoke群 | Review worktree Browser 69 files / 635 tests green。WPF focused store/UI smoke green。final focused/full normal-scaleはpending |
| type/lint/build | `pnpm typecheck`, `pnpm lint`, `pnpm build` | `dotnet build -c Release` | Review worktree Browser typecheck/lint/build green、WPF 0 warning・0 error |
...
| Modal Filmstrip / context actions | `ModalFilmstrip.test.tsx`, `ImageModal.test.tsx`, settings tests | `verify-wpf-modal-interaction.ps1` | vertical bounded rail、context Favorite action、chrome/edge/control contractはfocused green。isolated Browser/WPF runtime pending |
| thumbnail status borders | status schema/API/Settings/Grid tests | `verify-wpf-thumbnail-status-borders.ps1` | solid cyan + legacy rainbow migration、dirty preference merge、unknown/malformed/busy保護はfocused green。final normal-scale full/runtime pending |
...
| local API/public boundary | loopback `dev`/`start`、mutating API guard tests、isolated runtime | Browser API delegationだけ | direct argv fixは採用。guard tests、runtime、`LICENSE`決定、別setupのfull Codex Security scanなしではpublic readiness **NO-GO** |
| Section 14採用/残件 | 14.1は完了済みadoption、14.2はcurrent review delta、14.3は未完 | vertical Filmstrip/Settings/gallery/context/Album hardeningはin progress。Ctrl+C image copyはpending | focused greenをnormal-scale full/runtime/final SHAへ読み替えない。20k/100k/aggregate trackingは今回gateからユーザー明示除外 |

...
`pvu_view` はviewMode、thumbSize、aspectMode、displayStyle、sidebar/right panel、sort/random、folder sort、modal edge ratio、queue、manual dates、hidden folders、Unseen dots、Folders collapseをfield-localにnormalizeする。現行Browser mainのthumbSizeは20〜600、default 200、20px step、columnsは常に0/auto。600 endpointだけはavailable widthに関係なく1列へ解決する。旧保存値は20〜600へclampする。
...
ViewerState v2はLastFolder(Set)、SearchQuery、SelectedPath、CardWidth、right panel、style/aspect、sort/random seed、manual date、Favorite filters、Unseen dots、ConfirmBeforeDelete、Folders collapse/hidden/selection、preview tabs/pins、editable KeyBindings、unknown extension fieldsを保存する。
...
Browserはcurrent search query、current selection/primary、active modal、画像ごとのOriginal/Enhanced表示選択をreload restoreしない。WPFはSearchQuery/SelectedPathを保存するが、reloadでModalを自動openしない。両surfaceとも一時zoom/pan/flip、open dialog、in-flight request、hover popupを保存しない。
...
fixed keysはEscape close、Left/Right navigation、Favorite increase/decrease、configured Delete/Enhance/flip/zoom、E toggle、Space metadata sidebar、Enter external open。input/overlayでは背面shortcutを抑止する。

重要: 現行mainのEnter external openはsource image IDを `/api/open` へ渡す。表示中Enhanced outputを開く契約ではない。表示中Original/Enhancedを開く要件はSection 14のPENDINGである。
...
ModalのOriginal/Enhanced `E`、metadata sidebar `Space`、external open `Enter`、preview tab reopen `Ctrl/Cmd+Shift+T`は固定actionである。Grid/ListではArrowがselection、Spaceがselection、現行EnterがModal openを担うが、新採用要件としての一覧EnterはSection 14に隔離する。Ctrl/Cmd+1〜5はFavorite exact level。
...
- folder collapseは`aria-expanded`、Searchはcombobox/listbox/option + active descendant、imageはalt、icon-only controlは`aria-label`を持つ。
...
`Open externally` と `Show in folder` はactive root/catalog/type/existence/canonical guardを実行し、shell文字列結合を使わない。現行 `TryOpenSelectedExternally` はsource pathを開くため、表示中Enhancedを開く要件はPENDING。
...
Open externalとExplorer revealはDeleteと同じpath guardを再実行し、shell command文字列を連結しない。現行Browser/WPFのOpenはsourceを対象にする。表示中Enhancedを選ぶ新要件はSection 14の実装確認待ち。
...
| concurrent write conflict | lock/tmp + merge/retry | lock/tmp + generation writer | timeout後に未保存を明示しunknown fieldを保持 |
...
| `/api/open` | POST 200 | guarded source external open。shell文字列連結禁止 |
...
2026-07-19にlocal `refs/heads/main`は `53dc865c08e5009b511e39f0f23405e15730b3db` へfast-forwardされた。下表のimplementedはこのcommitのsourceと採用gateへ限定する。
...
採用gateはBrowser unit 59 files / 500 tests、build、lint、isolated Playwright 3/3、WPF aggregate 48/48（268,242ms）、exact 100,000 images / 100 folders green。Browser通常runtimeは同revision、`127.0.0.1:3000`、verifier PASS。WPFはRelease rebuild 0 warnings / 0 errors、startup shell ready 517ms、launcher revision/content provenance current、対話process PID 44540 / `PhotoViewer` / Responding=Trueまで同revisionで確認済み。
...
| search/tags/folders | `src/components/SearchBar.tsx`, `src/components/Sidebar.tsx`, `src/app/api/search/route.ts`, `src/app/api/tags/route.ts`, `src/app/api/folders/route.ts` | `local-native/PhotoViewer.Wpf/MainWindow.xaml.cs`, `local-native/PhotoViewer.Wpf/MetadataIndexStore.cs` |
...
| Modal | `src/components/ImageModal.tsx`, `src/lib/modalNavigation.ts` | `local-native/PhotoViewer.Wpf/MainWindow.xaml`, `local-native/PhotoViewer.Wpf/MainWindow.xaml.cs` |
...
| unit/component | `pnpm test:unit` | headless app smoke群 | Browser 59 files / 500 tests green at `53dc865...` |
| type/lint/build | `pnpm typecheck`, `pnpm lint`, `pnpm build` | `dotnet build -c Release` | `53dc865...`はBrowser lint/build green、WPF 0 warning・0 error。Browser typecheckは直前baseline記録greenで、今回の採用通知に独立再実行の記載なし |
| Landing/recent/runtime | `e2e/home.spec.ts`, `verify-browser-runtime.ps1` | landing/recent verifier、launcher freshness | Browser isolated/normal production Playwright各4/4、live runtime read-only PASS |
...
| reload/state | component hydration/session recovery tests | state/reload aggregate | WPF aggregate 48/48、268,242ms記録済み |
...
| Section 14採用/残件 | 14.1は`53dc865...` gate、14.2は専用test未完 | Favorite source confirmationは採用、他は未完 | surface単位のstatusを混ぜない |
...
4. Favorite/Seen/Recentをshared writerへ接続し、malformed/concurrent/restartを通す。

...
| Browser/WPF shared Search History v1 | focus/click list、whole-query replacement、delete/clear、API、protected status、同一process FIFO | nonblocking popup、keyboard/list selection、delete/clear、protected/Busy status | **implemented**。version 1/max50/NFKC共通identity、unknown保持、cross-runtime lost 0、Browser同一target 30並列全200/lost 0、WinForms非参加 |
...
| Search History | route/SearchBar tests、API live-lock timeout、同一target FIFO | `verify-wpf-search-history.ps1`, `verify-cross-runtime-search-history.ps1` | async UI、keyboard/a11y、max50、malformed/future保護、Busy writes 0、cross-runtime lost 0、Browser 30並列全200/lost 0 |

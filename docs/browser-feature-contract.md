...
4. `next start --hostname 127.0.0.1 --port <port>` をparent-watch wrapperで起動する。
5. ready後にbrowserを開く。

3000は既定候補であり、競合時に必ず3000へ固定する契約ではない。空port判定はIPv4/IPv6の片方だけをprobeせず、どちらかに既存listenerがあれば次portへ進む。`PVU_NO_OPEN=1` は検証時にbrowser自動openだけを抑止できる。
...
- client connection切断、同一clientの新scan、Landingへ戻る操作は現在runをcancelする。cancel済みrunはpartial cacheとprocess-global indexをpublishしない。
...
UIはfolder progress、処理数、全体percentを表示する。connection lost、malformed event、same-set conflictはblocking alertではなくLanding内のinline errorへ遷移し、原因、Retry、Dismissを表示する。Retryは直前のfolder setを再利用し、Dismissは入力済みfolder setを保持する。
...
- 成功pathだけをstoreの通常Delete reconciliationでselectionから除外し、失敗pathは選択に残して再試行可能にする。partial/all failureで全selectionをclearしない。
...
- launcher外から起動したserverでは取得不能fieldをnull/falseで明示する。
...
11. Bulk partial failureで成功分だけ消え、失敗画像は選択に残る。progress/final success+failure countを画面と`aria-live`へ出し、連打で二重実行しない。
...
現行BrowserはGallery primary button/group、folder controls、Search combobox/chips、preview tabs、dialog、resize separatorへkeyboard/semantic contractを実装済み。Touch-only chip dragは残差だが、keyboardだけで同じreorder/remove結果へ到達できる。
...
9. queued enhancementはrestart直後に自動resumeしない。
10. Comfy cancelはprompt ID取得前だとglobal interruptの曖昧性がある。
15. shared JSON lockはtransaction log/revision IDを持たず、index/thumb/enhancement jobはFavorite等と同じthree-way contractではない。
...
26. Search chip reorderはtouch dragだけでは完全操作できない。
27. lifecycle keepaliveはbrowser全体の約64KiB上限を考慮して60KiB以下だけ送る。oversizeまたは既存PUT中にcloseした場合、同じsession内のshared完了は保証せず、durable pending journalを次回hydrationで再送する。
...
| --- | --- |
| Runtime/launcher | `package.json`, `scripts/prod_launcher.js`, `scripts/serve_with_parent_watch.js`, `scripts/verify-browser-runtime.ps1`, `src/app/api/runtime/route.ts` |

...
- speculative warmupは全Sharp slotを占有しない。concurrency 1〜5では1枠、6以上では2枠をdirect visible/focused response用に予約し、direct requestが無い間は予約枠を空けて即応可能にする
- queued warmupがdirect responseへpriority upgradeされた時は、予約枠へ即時dispatchする。warmup完了後は残りqueueを再開し、eventual drainを維持する

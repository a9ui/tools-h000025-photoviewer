...
検証agentはユーザーが使用中のport 3000 ownerを開始前後に記録し、別の明示free portを使う。停止権限は自分が起動したexact launcher PIDと、そのlauncher/wrapperが直接生成したserver process treeだけに限定する。port番号、同名`node.exe`、project root文字列の部分一致だけを所有根拠にしてはならない。`scripts/verify-browser-runtime.ps1`はlistener/runtimeをread-only検証するもので、無関係processの停止権限を与えない。
...
- Shared `.cache/favorites.json`をBrowser/WPF共通のexact正本とし、共有済みpathはlevel減算と0 clearもそのまま反映する。Browser localStorageは即時UI、offline retry、crash recovery用mirrorであり、古い高levelをshared値へmax mergeしない。
- Shared persistence導入前のBrowser local-only pathは、一度だけsharedに存在しないkeyを非破壊importする。成功確認後に`pvu_favorites_shared_migration_v1`を記録し、以後はWPFの減算/clearをlocal mirrorから復活させない。malformed/unreadable local snapshotではmarkerを記録しない。
- 現sessionで変更したdirty pathとpending journalはlocal exact値を優先し、遅いserver GETで削除や減算を復活させない。
- 起動時に加え、window focusまたはvisible復帰時にshared mapを再取得する。常時pollingはしない。refreshとshared PUTが競合した場合はwrite generationとdirty overlayで新しいlocal intentを維持する。
...
- pending journal v2はdirty pathの`desiredFavorites`も保持し、primary欠落やstale backupがあってもexact clear/decreaseを復元する。desired値を持たない旧v1 journalは、対応するprimary mirrorが正常に読める場合だけ互換replayする。primaryがmalformedまたは欠落してdesired値を証明できないv1はbyte-preserving quarantineとし、自動PUT、migration、削除をせず、次の明示的Favorite変更でv2へ置換する。
...
- WPF legacy fileとの読取互換として、stored mapだけはboolean、Int32、`"true"`/`"false"`文字列を受け、truthyだけをSeenへ正規化する。Browser PUTは引き続き`true`だけを受け、次の保存はcanonical true-marker mapにする。
...
- Favorite/Unrated/Enhancedのclient filterでは、viewportとoverscanを満たすmatch bufferを目標に追加pageを漸進loadする。
- Filtered listの末尾へ到達した時の追加需要は、次batchでmatchが増えて旧scroll位置が末尾でなくなっても保持する。Sparse batchが目標bufferを満たすかsource totalを走査し終えるまでbounded page loadを継続し、最終pageのmatchにも到達可能にする。
- この継続loadはactive client filterだけで動く。filter/query/index-token context変更またはfilter解除で旧需要を破棄し、通常閲覧で100k件metadataを無条件loadしない。
...
- range 40〜600
- default 200
- Grid表示中のmodified wheelとkeyboard zoomはviewer全体でgallery zoomとして捕捉し、Browser page zoomへ渡さない。これによりSidebar、header、text、right panelの見かけの大きさを固定する。
- `+` / `-` は20px step、`0`は200pxへresetする。text input、textarea、select、contenteditable、Modal、Settings、Confirmでは捕捉しない。
...
- Grid viewerがactiveでないLanding、List、Modal、Settingsではnative shortcutを保持する。
- Plain wheelとAlt+wheelはGridでもnative scrollへ渡す。

Sidebar、header、text、right panelのCSS sizeはgallery zoomで変えない。

### BR-GAL-007 Zoom anchor

Zoom開始時、viewport内に最後の選択画像があればその画像とviewport offsetを最優先anchorにする。選択画像が見えていない場合はwheel cursor直下の画像、画像外またはkeyboard/slider操作ではviewportのhorizontal centerに最も近い表示画像をanchorにする。reflow後、同じ画像がほぼ同じoffsetへ来るようscrollを補正し、scroll boundsへclampする。

受入許容:

- anchor画像が残る場合、reflow後もviewport内に見える。
- viewport内の選択画像はcursor位置や操作元がSidebarでも選択前のoffsetを優先する。
...
Plain wheelとAlt+wheelはnative scrollへ渡し、Gallery zoomを発火しない。window capture handlerはGrid viewerがactiveな時だけCtrl/Cmd+wheelとCtrl/Cmd+`+`/`-`/`0`を処理する。
...
| `pvu_favorites_shared_migration_v1` | local-only Favoriteのshared import完了marker | successful one-time import / no-op確認 |
...
Favorite変更はbrowser localStorageと`pvu_favorites_pending`へ即時mirrorし、通常は300ms debounce後にdirty pathだけをbase→incoming three-way PUTする。`pagehide`、hidden `visibilitychange`、unmountでは、UTF-8 bodyが60KiB以下かつ別writeがin-flightでない場合に`keepalive` PUTを開始する。Keepaliveはbrowser全体の概ね64KiB queue制約があり完了通知も保証されないため、oversize、in-flight、network failure、close後にresponseを観測できない場合はjournalを残す。次回hydrationはshared exact mapをbaseにし、journalのdirty pathだけをlocal exact値として優先して同じdeltaをidempotentに再送する。遅いresponseはjournal revisionが一致する時だけackし、newer changeを巻き戻さない。先行PUTの成功後にnewer revisionがある場合は、そのserver responseを次のthree-way baseへrebaseしてから直列再送する。SeenもlocalStorageへ即時mirrorし、pending additive stateを同じlifecycleでbounded keepaliveする。

Value shapes:

- `pvu_favorites`, `pvu_favorites_backup`: `Record<absolutePath, 1 | 2 | 3 | 4 | 5>`。
- `pvu_favorites_pending`: `{ version: 2, revision, dirtyIds, baseFavorites, baseKnownIds, desiredFavorites }`。`desiredFavorites`にdirty IDが無い場合はexact clearを表す。旧version 1は上記の安全条件だけで読取互換とする。
...
env overrideがないFavorite、Seen、Recent folders、Enhancement jobs/outputsは、linked worktree起動でも`.git/commondir`から通常main checkoutを解決し、同じshared `.cache`へ収束する。worktree固有のinvisible second historyを作らない。
...
- missing path: 400
- `indexToken`なし: 403。implicit active indexへfallbackしない
- expired `indexToken`: 410。別のactive indexへfallbackしない
- token付きsession index外: 404。fallback active indexへescapeしない
- same-origin browser `<img>`の`no-cors` GETは、loopback Host、`Sec-Fetch-Site: same-origin`、`Sec-Fetch-Dest: image`、Origin absentまたはHost一致の場合だけProxy/routeを通す。その他のno-cors/cross-origin requestは403
- missing file: 404
- unsupported: 415
- stream: 200
- stream responseは`Cross-Origin-Resource-Policy: same-origin`と`X-Content-Type-Options: nosniff`を返す
...
- env overrideがない場合、linked worktreeから起動しても`.git/commondir`を辿った通常main checkoutの`.cache/favorites.json`を使い、WPFと履歴を分岐させない。
...
- `GET /api/seen`: `{ ok, seen, malformed, error? }`。file不存在はempty true-marker map。WPF legacy markerは読取時だけbooleanへ正規化する。
- env overrideがない場合はlinked worktreeからも通常main checkoutの`.cache/seen.json`を使う。
...
- env overrideがない場合はlinked worktreeからも通常main checkoutの`.cache/recent-folders.json`を使う。
...
env overrideがないjob storeとmanaged outputsはlinked worktreeからも通常main checkoutの`.cache/enhance/**`を使う。通常閲覧でjobを作成・開始しないownership境界は変えない。
...
- focused/visibleだけを即時flushし、同じbatchに入っていたnearbyは40ms timerへ残す
- server待ち行列ではdirect image responseを同priorityのwarmupより先にし、visible/nearbyは新しいviewportを優先する
- pending speculative warmupは最大1200。超過時は低priorityかつ古いworkからdropし、direct responseはtrim対象にしない
...
- CPU基準で4〜12を目安
- `PV_THUMB_CONCURRENCY` のeffective rangeは2〜16。1以下は2へclampし、最低構成でもdirect用の1枠を確保する
- speculative warmupは全Sharp slotを占有しない。concurrency 2〜5では1枠、6以上では2枠をdirect visible/focused response用に予約し、direct requestが無い間は予約枠を空けて即応可能にする
- queued warmupがdirect responseへpriority upgradeされた時は、予約枠へ即時dispatchする。開始済みのdirect/Sharp jobはpreemptせず、有限のdirect負荷が収束した後に残りのwarmup queueをdrainする
...
Browser static gateは次を同一checkoutで実行する。
...
test file/test件数はsuite成長に伴って増える観測値であり、固定の合格件数ではない。0 failureを要求し、実行時のpassed/skipped件数と意図的skipの理由を記録する。古い件数へ合わせるためにtestを除外・削除してはならない。
...
7. 開始前後でユーザーport 3000のowner PID/command lineが同一で、isolated runtime停止は記録したexact launcher PID/treeだけを対象にする。
...
- Browser local-only legacyを1回sharedへ移行した後、WPFでLv5→Lv1→Lv0と変更し、Browser focus/reloadで1→0をexact反映して古いlocal Lv5を復活させない。
- 通常checkoutとlinked worktreeのBrowser/WPFが同一shared Favorite pathを解決する。
...
次は「現行挙動の記録」であり、そのまま再現するMUSTではない。このSectionまたは本文で明示的に`CURRENT LIMITATION`と書いた項目だけを既知制約として扱う。NON-GOAL、意図的DEFER、WPF native extensionをBrowserの欠陥や暗黙要件へ読み替えない。

...
2. 引数なしでは、Windows上で同じproject rootを参照する古いproject server processを安全に整理し、port 3000から3999までの最初の空portを選ぶ。
...
5. `next start --hostname 127.0.0.1 --port <port>` をparent-watch wrapperで起動する。
6. ready後にbrowserを開く。
...
| --- | --- |
| Runtime provenance / launcher CLI safety | `src/app/api/runtime/route.test.ts`, `scripts/prod_launcher_cli.test.mjs`, `scripts/verify-browser-runtime.ps1` |

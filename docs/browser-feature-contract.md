...
- client cancelはrun generationを先に失効させ、progress/errorをclearし、folder setと既存recent/state/cacheを保持する。cancel後の遅延`progress` / `complete` / `error` / connection errorは全て無視し、二重Cancelはno-op、直後の再scanを許す。
...
2. progress受信後にCancelするとEventSource close 1回、phaseは即Landing、progress/errorはclear、入力済みordered folder setと既存recent/state/cacheは不変。

...
- valid mapはlevelを0〜5へnormalizeし、temp fileからrenameして全置換する。200 `{ ok: true, favorites, malformed: false }`。
...
29. Sparse resultのrange/keyboard/Delete neighborはloaded subsetに制限される。
30. Current result外のPreview tab clickはmodalを開かない。
31. Favorite unmount flushはbrowser localStorageだけで、pending shared PUTを完了させない。
32. thumb/display/enhance output disk cacheにquota/LRU evictionがない。

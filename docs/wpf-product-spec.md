...
- Modalの`Open externally`も同じactive catalog/root/type/existence/canonical guardを起動直前に再実行し、`UseShellExecute`へcanonical pathを1件だけ渡す。未関連付け、ShellExecute拒否、I/O/access/path failure、起動直前のsource消失はUI dispatcherへ逃がさずgeneric Retry statusへ変換し、selection、Modal、focus、source/shared stateを変更しない。
...
現行ledgerには、監査した主要journeyの既知P0/P1実装残はない。100,000件/2.9〜3.2 MiBで再現していた同期shared Favorite/Seenのdispatcher停止はgeneration-aware single-writer actorで解消した。最終jitter-margin 3回gateはlarge modal next p95 7.159/9.803/12.169ms、Favorite p95 45.148/41.155/42.445ms、dispatcher max gap 50.561/50.412/55.704msで、50/65/110ms絶対閾値とnoise-aware relative gateをすべて満たした。coalescing、external entry保持、stale completion、fault rollback/Retry、malformed保護、close drain/reopen、self-test、shared state/recent cross-runtime各20反復もgreenである。standalone 20,000 catalogはexact 20,000、silent truncate 0、Grid max 384、List 22で、far-tailのcanonical/visual selectionとGrid→List→Grid roundtripがすべてtrue、mode roundtrip 32ms、dispatcher max gap 292ms、external probe 254msを記録した。aggregate内の同gateもexternal 248ms以下でgreenだった。FoldersはUI Automation `ExpandCollapse`によるExpanded→Collapsed→Expandedとvisual/persistence migrationをfocused gateで通している。

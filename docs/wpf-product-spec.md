...
- custom maximizeは現在windowが属するmonitorのtaskbarを除いたwork areaを使う。primary monitorへ飛ばさず、mixed-DPIではdevice pixelをWPF DIPへ変換し、monitor API失敗時だけ`SystemParameters.WorkArea`へ安全にfallbackする。restoreはmaximize前のboundsをexactに戻す。
...
| Current-monitor maximize/restore | `powershell -File scripts/verify-wpf-monitor-work-area.ps1` |

...
cleanupする。失敗時はtombstoneとpending statusを保持し、latest documentへretryできる。`addToAlbum` shortcutは既定Bだが、既存Filmstrip
Bとunknown future actionに保存されたvalid chordも予約済みとして扱い、空いているfallbackへ割り当てる。

独立review hardeningはlocal `main` HEAD `f64d98d81e016e43177b1e39738f759742d1215b`
上の未commit worktree deltaである。Browser unit 69 files / 635 tests、
typecheck/lint/build、WPF Release buildとfocused WPF verifierはgreenだが、
final focused/normal-scale full、Browser/WPF runtime/provenance、local API guard
tests、final SHAは**PENDING FINAL GATE**であり、本書のworking-state記述を完成
宣言に使わない。20k/100k catalog-scaleとaggregate check-count trackingは
ユーザーが今回のreview-hardening gateから明示除外した。下記historical green
recordsは変更せず、current deltaへ横流しせず、今回rerunも要求しない。

Browserのdirect argument-vector external openは採用済みである。明示loopback
`dev`/`start`とmutating API Origin/Host guardはworktree実装中でtests pending。
`LICENSE`未決かつfull Codex Security scan未実行のためpublic readinessは
**NO-GO**で、WPFを含む製品はprivate/local・deployment禁止のままとする。scanは
外部相談禁止の本laneではなく、別途authorizeしたsetupで扱う。
...
- focus/hover/selectedは既存Accent/GlassBorder/HoverFill tokenを使う。すべてのenabled button styleはpointer hoverとkeyboard focusで見た目が変わり、disabled stateではhoverを出さない。ToolTipはdark surface、non-empty text、明瞭なforegroundを使う。
- logoはindigo→purple gradient。Favoriteはrose系。dangerはred。新しいaction色を増やさない。
- landingは中央760pxのfolder-set panel、52px logo、local metadata説明、左右の低opacity radial glow、右上のApp Settings入口とminimize/maximize-or-restore/close caption controlsを持つ。
...
Landingにも同じminimize、maximize/restore、close actionを置き、custom chrome上でclick可能にする。
...
8. Original/1:1/2:3
9. Gallery zoom
10. App Settings（scroll contentの後ではなくSidebar下端へ固定）
...
- Grid/List itemはdirect Favorite +1/-1 controlを持ち、selection/virtualizationを壊さずshared Favorite writerへ送る。
- small card endpointはneutral backgroundとbounded uniform geometryを使い、filename/cardが過度に細長くならない。rainbow art backgroundを使わない。
...
- Ctrl/Win+wheelはGrid galleryがactiveで、Settings/Delete overlayがなく、pointer/focus sourceがTextBox、ComboBox、DatePicker、Buttonでない時だけgallery zoomへ渡す。Cards surfaceとそのvertical scrollbar由来を同じgallery inputとして扱い、Landing、List mode、入力control上ではnative wheelを維持しcard widthを変えない。
- middle-clickはCards/Rows surfaceだけでauto-scrollを開始し、pointer displacementで連続scrollする。再middle-click、Escape、capture loss、surface終了で停止する。source file、selection、Favorite、Seenを変更しない。
...
- pan、horizontal flip。
- Original/Enhanced表示切替。成功済みmanaged outputだけを候補にし、存在する場合はEnhancedを既定表示する。Original/Enhanced controlとEで切り替え、切替自体はjobを作らない。
...
- image areaはwindow全体を使い、画像を`Uniform` fitでtoolbar/Filmstrip以外の利用可能領域へ最大表示する。left 28% / center 44% / right 28%をpointer zoneとし、left/rightはprevious/next、centerはimageまたはempty areaのchrome toggleで、各zoneはAutomationName/HelpTextを持つ。黒いbackdropは一覧へ戻し、topbar/footer/control clickはbackdropやchrome toggleへ伝播しない。旧circle-only previous/next buttonをnavigationの主契約にしない。
...
- filtered/sorted full orderのFilmstripを持つ。manual chrome表示中は左の専用vertical rail、manual hidden中はpointerが左端へ入った時だけ前面vertical railとして表示し、離脱時に畳んで画像geometryを変えない。currentへ追従し、clickで直接選択する。`T`/toolbarで開閉し、`ViewerState.ModalFilmstripOpen`へ保存する。
- zoom倍率はModal root最上端へ1つだけlow-contrast indicatorとして表示し、画像・topbar・Filmstripへ重ねない。重複するzoom labelを置かない。
...
- Prompt metadataはcomma区切りをtrimし、emptyとcase-insensitive duplicateを除外したstable-order chipとして表示する。`((...))`、`[...]`、`{...}`のemphasis-only wrapperを除き、Browserと同じplain tagとして表示する。
...
- image areaのright-click product menuはFavorite +1/-1、Original/Enhanced、guarded external open、Show in folder、Album add、Filmstrip、確認付きsource Recycleを提供する。menu actionは既存guard/action handlerを再利用する。画像bitmap/file clipboard copyはまだ提供しない。
...
Thumbnail status bordersはFavoriteを既定ON/`#facc15`の2px inner、AI-enhancedを既定ON/`#38bdf8`のthin cyan outerとして独立設定する。旧`rainbow` inputはsolid cyanへmigrationし、rainbow UI/backgroundを復活させない。WPFは変更したFavorite/Enhanced preferenceだけをlock内latest `.cache/settings.json`へmergeし、別surfaceが同時更新したもう一方を古いdraftで戻さない。dirty preferenceなしの保存はno-opまたは保護中Retry案内で、empty mutationを作らない。unknown field、旧Enhanced hex、malformed/future、fresh busy lockを非破壊で扱う。
...
| AI-enhanced thumbnail border | ON / `#38bdf8` solid cyan |
...
Ctrl/Win + wheelのgallery zoomはpointer gestureなのでKey bindingsの対象外だが、sidebar/textのscaleを変えず同じgallery zoom modelを使う。Win chordはeditable key bindingには採用しない。`Add selection/current image to Album`はBを優先するが、既存known bindingとunknown future actionに保存されたvalid chordを予約済みとして扱い、空いているfallbackを割り当てる。`Toggle Original / Enhanced`は既にsucceededでmanaged outputが存在する時だけ表示sourceを切り替える。key設定にjob create、worker start、暗黙Enhancement actionを追加してはならず、明示`AI x2`だけがSection 13のcreate routeを使う。
...
| Gallery zoom / one-column / geometry anchor / surface-wheel / middle auto-scroll | `powershell -File scripts/verify-wpf-gallery-zoom-anchor.ps1` |
...
| Modal pointer/chrome/full-window/vertical Filmstrip/context action | `powershell -File scripts/verify-wpf-modal-interaction.ps1` |
...
| Modal prompt tag to search / emphasis-wrapper normalization | `powershell -File scripts/verify-wpf-prompt-tag-search.ps1` |
...
| Sidebar/App Settings synchronization / landing chrome / single scroll / backdrop / hover | `powershell -File scripts/verify-wpf-settings-unseen-dots.ps1` |
...
Historical base `a091ec7`はBrowserと同じshared `thumbnailStatusBorders` schemaと当時のrainbow optionを採用し、後続`60ed739`はdirty Favorite/Enhanced preferenceだけをlock内latest diskへmergeして他surfaceの同時更新を保持した。current review hardeningはrainbow presentationを廃止し、legacy `rainbow`をsolid cyan `#38bdf8`へmigrationする。focused verifierはmigration、reload、lost update、unknown fields、malformed、busy lock、残骸0をgreen、Release 0 warning / 0 errorで確認した。final normal-scale full/runtimeはpendingである。
...
Review hardening worktreeでは、Landing caption controls、sidebar下端Settings、single-scroll/backdrop Settings、enabled-only hover/dark tooltip、Grid/List direct Favorite、active Grid source/scrollbar Ctrl+wheel、Grid/List middle auto-scroll、bounded small cards、full-window Uniform Modal、28/44/28 edge、left vertical Filmstrip、image/empty chrome toggle、Enhanced default/Original toggle、functional top controls、context Favorite action、single zoom indicator、prompt wrapper normalizationをfocused smokeでgreenにした。Browser unitは69 files / 635 tests、typecheck/lint/buildもgreenである。ただし最終focused/normal-scale full、Browser isolated runtime、normal WPF runtime/provenance、loopback/Origin-Host guard tests、final SHAは**PENDING FINAL GATE**である。20k/100kとaggregate check-count trackingはユーザー明示除外のため今回rerunせず、過去数値はhistorical evidenceのままcurrent deltaへ横流ししない。

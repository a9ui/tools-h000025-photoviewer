...
| Explicit scan cancel parity | **BROWSER DONE / WPF FOLLOW-UP** | BrowserはScanning中の`Cancel scan`でEventSource/requestをabortし、generation失効、Landing復帰、partial index非公開、即再scanを保証。WPFは内部`_loadCts`による置換cancelはあるが、利用者向けCancel surfaceと同等acceptanceは未実装。P1 `ADD`。 |

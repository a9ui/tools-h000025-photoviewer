...
このledgerは2026-07-19のlive code、50-check aggregate、exact 100,000 images / 100 folders cold→warm gate、cross-runtime verifierを照合した状態。後続変更では該当row、正規仕様、focused verifierを同じchangeで更新する。
...
| Persistent Prompt metadata index | **DONE** | WPF専用のversioned/checksummed folder-set indexを導入。source identity一致だけをhitにし、partial staleは該当entryだけ更新、corrupt/malformedはsource fallback、future schema・decode failure・cancelはlast complete bytesを保持する。restart all-hitは無書換えで、Viewer sidebar/footerへ進捗とreused/refreshed結果を表示。Favorite/Seen/Recent/Enhancement/Browser cacheは非所有。`verify-wpf-metadata-index.ps1`, `verify-wpf-catalog-stress.ps1` |
...
- dimension/Promptの完了snapshotはWPF専用のsource-identity付き永続indexへ保存し、restart all-hit時はsource metadataを再読込・index再書込みしない。破損・部分失敗・future schemaはsource/last-completeへfail safeし、Browser共有state/cacheを回復目的で削除しない。

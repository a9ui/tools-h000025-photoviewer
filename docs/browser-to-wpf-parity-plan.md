...
このledgerは文書基準commit `64472dd`のlive codeとverifier inventoryを照合した状態。後続変更では該当row、正規仕様、focused verifierを同じchangeで更新する。
...
| WPF normal launcher ownership | **DONE** | `start_wpf.bat`はRelease targetのmissing/staleだけをrebuildし、current targetを直接起動。Browser server/port 3000/既存WPF processを探索・停止しない。`verify-wpf-launcher-freshness.ps1` |
...
| Folder switch/Refresh soak | **DONE** | 1,000枚×2 folderを同一processで40 cycle切替。metadata/preview/modal遅延、explicit cancel 14、supersede 26を混ぜ、stale completion 0、load CTS 121/121、最終catalog/selection/modal/tabの最後folder所有、Grid/List bounded、shared JSON/source byte不変、Enhancement 0を固定。`verify-wpf-reload-soak.ps1 -Count 1000 -Cycles 40` |
...
| Explicit scan cancel parity | **DONE** | WPFもenumeration/metadata中だけ`Cancel scan`を表示。current generation失効、ordered draft保持、Landing復帰、polite status、Open focus、double-cancel no-op、partial state/recent/catalog非公開、遅延cancelより即再scanを優先。`verify-wpf-scan-cancel.ps1` |
...
| 明示scan cancel | `ADOPT` | P1 | enumeration/metadata限定Cancel、generation guard、Landing draft/focus/status復帰。`MainWindow.xaml`, `MainWindow.xaml.cs`, `verify-wpf-scan-cancel.ps1` | cancel runはcatalog/current/state/recent/Seenをpublishせず、double cancelはno-op、遅延完了はnewer rescanを上書きしない |

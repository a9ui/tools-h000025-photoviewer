...
- Browser `pvu_view.sidebarOpen` is Row 22 `DEFER`: browser persists the
  left-sidebar expanded/collapsed flag, but the current native local surface
  has no accepted left-sidebar collapse persistence target. The value remains
...
- Browser `pvu_view.modalEdgeRatio` is Row 23 `DEFER`: browser persists the
  modal edge-click ratio, but native has no accepted modal edge-zone
  persistence target. The value remains only inside the raw
...
- Browser `pvu_view.enhanceQueueOpen` is Row 24 `DEFER`: browser persists the
  enhancement queue panel open/closed flag, but native explicit enhancement
  queue/settings UI remains owned by #97/#98 and ordinary native browsing must
  not start enhancement workers. The value remains only inside the raw
  `browser_pvu_view` mirror, and native does not create `enhance_queue_open`,
  `enhanceQueueOpen`, `enhancement_queue_open`, `enhance_queue_visible`, or
...
| `pvu_view.enhanceQueueOpen` | `native_settings.browser_pvu_view` raw mirror only | `DEFER`: formally covered by Row 24; native enhancement queue open/closed persistence belongs to #97/#98 explicit enhancement UI. |
...
| `pvu_view.enhanceQueueOpen` | `DEFER` | Formally covered by Row 24: browser enhancement queue panel state belongs with #97/#98 explicit enhancement UI and remains raw-mirrored only. |
...
`pvuEnhanceQueueOpenDeferred=true`, `pvuSortDirectionSeedDeferred=true`,
...
`left_sidebar_open`, or `left_panel_visible` native settings. Row 23 keeps
...
`modal_edge_zone_ratio`, or `modal_click_edge_ratio` native settings. Row 24
keeps `enhanceQueueOpen` only inside raw `browser_pvu_view` and does not create
`enhance_queue_open`, `enhanceQueueOpen`, `enhancement_queue_open`,
`enhance_queue_visible`, or `enhancement_queue_visible` native settings.
...
- `DEFER`: browser `pvu_view.enhanceQueueOpen` as a native migration target.
  ...
  not create native enhancement queue-open settings; #97/#98 own explicit
  enhancement queue/settings UI.

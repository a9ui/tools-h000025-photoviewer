...
M8 first slice implements native controls for folder visibility buckets,
sorting/display, and preview-panel visibility/details. It does not claim full
native parity.

Decision: `NATIVE_PARITY_SLICE_ADDED_WITH_REMAINING_GAPS_DEFERRED`.
...
- The browser app remains preserved; M8 first slice does not require `src/**`
  changes.
- Rows that still need product implementation, screenshot polish, destructive
  manual checks, or broader browser equivalence stay `DEFERRED` in
...
- Fixture preparation passed:
  ...
  - created state:
    ...
    `.cache\native\browser-localstorage-export.json`
  - skipped existing state: none in this isolated M8 worktree
...
## Native Rows Advanced By M8 First Slice

M8 adds native acceptance evidence for:

- folder visibility bucket rendering and all/none filtering,
- name sort, random sort mode, and reshuffle command path,
- thumbnail-size display control,
- preview panel show/hide control,
- preview detail show/hide control,
- keybinding metadata for `Ctrl+P`, `Ctrl+D`, and `Ctrl+R`,
...
- multi-root folder sets, remove-folder behavior, folder range selection,
  ...
  separate detail modal zoom/pan/flip, bulk preview actions, full settings

...
WPF版は「作りかけのUI shell」段階を脱し、Browser契約のP0〜P2を満たす実用viewerになった。5,000枚常用gate、20,000枚aggregate stress、exact 100,000枚/100 folders closeout gate、full-extent visible-only virtualization、非同期search/preview/hover/metadata、Favorite/Seen共有、Recycle-only Delete、state migration/error/a11y、bulk Favorite/Recycle、Folder range selection/collapse persistence、Prompt tag→search、Explorer FileDrop、明示Enhancement bridgeがfixtureで固定されている。以後は実操作visual、競合、性能stressで再現した欠陥を直す段階である。
...
19. Seen persistence refusal時に`tile.Unseen`だけがrollbackされ、青ランプの`ShowUnseenDot`がfalseのまま残る表示不整合を監査で発見した。両値を同じtransaction境界で復元し、malformed Seenを使うP1B smokeで選択中tileと全visible-dot countのrollbackを固定した。
20. 100,000件/2.9〜3.2MBのshared Favorite/Seenでmodal-next p95 186〜204ms、Favorite p95 233〜251ms、dispatcher最大停止442〜460msを再現し、UI-thread full-store persistenceへ局在させた。generation-aware single-writer actor、coalescing、latest-disk merge、fault rollback/Retry、close drainへ置換し、3反復の50/65/110ms絶対gateとnoise-aware relative gateをすべてgreenにした。
21. 100,000 pathを保持してもGridのscroll extentが96件ずつ・最大384件のmoving windowしか持たず、末尾到達が実用不能だった。全件ItemsSourceを`VirtualizingWrapPanel`へbindし、visible rows + 2 overscanだけをrealizeする構造へ置換した。exact 100,000枚/100 foldersでGrid/List 15/9 containers、末尾99,999、Created/Modal、List末尾thumbnail、zoom drift 0を固定し、metadataはViewer公開後のbackground streamへ移した。
...
| Large library | Green structure | Green at 100,000 / 100 folders | WPF catalog/filtered/Grid source 100,000、truncate 0、Grid/List realized 15/9、tail 99,999、Viewer ready 4,975ms、List tail thumbnail 165ms、zoom drift 0、liveness 750ms gate内 |
...
| Enhancement isolation | Green | Green explicit bridge | passive enqueue/worker 0。WPFはModalの明示AI操作だけをloopback Browser APIへ送り、create/cancel/retry/output deleteとstale response guardを持つ。native engine/worker ownershipはDEFER |
...
- WPF Release build、`verify-wpf-product.ps1 -IncludeReloadSoak`、exact 100,000枚/100 folders、cross-runtime Favorite/Seen/Recent。`-IncludeReloadSoak`は既定24-cycle soakをaggregate内で実行し、40-cycle以上は長期memory傾向のextended観測として分ける。2026-07-18最終runは47/47、264,857ms。check数はinventoryから動的に決まり、固定数ではなくlive JSONの0 failureをgateにする。
...
5. 最終aggregateは1,000枚×2 folderを同一WPF processで24 cycle切替/Refreshし、explicit cancel 8、supersede 16、stale completion 0、preview/modal 24/24、load CTS 73/73、shared JSON/source byte不変、Enhancement 0でPASSした。forced-GC managedは9,706,816→9,702,496 bytes、working setはwarm 177,762,304、final 194,555,904、max 212,271,104 bytesでmemory envelope内だった。先行extended 40-cycleは長期傾向の観測値として別に保持する。`verify-wpf-product.ps1 -IncludeReloadSoak`, `verify-wpf-reload-soak.ps1 -Count 1000 -Cycles 40`。
...
scan完了後の`File.Exists` snapshotから`FileInfo.Length` / timestampをTileへmaterializeする短い間にsourceが消えると、想定内の`FileNotFoundException`がUI dispatcherまで脱出し得るTOCTOUも再現した。IO/access/path/security failureだけを画像単位でskipし、survivor count、selection、Preview tab、Modal、state、scan warningと既存persistence warningを同時に整合する。`verify-wpf-scan-materialization-race.ps1`はsource/shared storesを変更せず、residue 0のtemp fixtureで固定する。

### P2-E Path robustness / mixed failure visibility

Unicode、emoji、空白、apostrophe、mixed-case extension/root、427文字absolute path、read-only、exclusive lock、corrupt、external disappearance、valid + unavailable multi-rootを同一temp fixtureでscanした。catalog/search/preview/modal/tabはpath identityを保ち、lock/corruptはplaceholder継続、unlock後Refreshは復帰、external disappearanceはstale surfaceを除去する。Explorerはinjected `ArgumentList`、drag-outは`DataFormats.FileDrop`、Deleteはprotected-root guard + fake backendだけで検証し、real Explorer/Recycle/source deleteは起動していない。root warning上書きを合成statusへ修正した。さらに`Open externally`の未関連付け、ShellExecute拒否、Win32/IO/access/path failure、起動直前source消失をgeneric Retryへ変換し、selection/Modal/focus/dataを維持するfocused gateを追加した。

100,000件/100 foldersの最終観測値（このmachine/fixtureの証拠であり製品hard閾値ではない）: Viewer ready 4,975ms、metadata background 28,367ms、full load 33,792ms、Grid/List realized 15/9、List末尾thumbnail 165ms、flat zoom 35/50ms、Created zoom 43/60ms、anchor drift 0、dispatcher heartbeat最大gap 584ms、external WM_NULL最大停止522ms、working set 143,036,416→321,769,472 bytes。source 100,000維持、truncate 0、Enhancement passive 0、TEMP cleanup成功だった。

### P3 製品判断後

- WPF単独Enhancement engine/worker ownershipと複数output version selector。明示create/cancel/retry/output deleteのBrowser API bridgeは実装済み。
...
ここだけが現行WPFのCURRENT LIMITATION / DEFER境界であり、P0〜P2 viewer coreの実装残ではない。Browser側のCURRENT LIMITATIONは`browser-feature-contract.md` Section 22に限定して列挙され、WPFへ欠陥として複製しない。Browserの代表残差はloaded subset外のgallery range/keyboard selection、surface別error通知、cache quota不在、local HTTP APIの認証/CSRF不在。WPFの代表残差はstandalone Enhancement ownership/version selector、cache quota、配布packageである。
...
- WPF `-IncludeReloadSoak` aggregateのlive check数/全result、cross-runtime 2本、24-cycle final soakと必要時の40-cycle extended soak、5,000/20,000/100,000件metrics

...
Favorite filter中のlevel変更後の即時再同期と、sparse paged resultのmodal/Delete full-order neighborは完了した。残りは次。

1. Delete成功後のFavorite/Seen/pin/enhancement orphan cleanup方針を確定する。

### P2-C 共通性能・回帰

1. rapid window/tab/filter/resizeでstale decode/state writeをstress。
2. Browser route worker + WPF writerのFavorite/Seen同時20回は完了。次はRecentを含む3-owner repeatへ拡張する。

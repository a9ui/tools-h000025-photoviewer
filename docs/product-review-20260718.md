...
| Packaging/operations | Green local launcher | Green local launcher | Browserは通常launcherのloopback/provenance/owned child-tree停止を固定。WPFは`start_wpf.bat`がmissing/stale Releaseだけをrebuildし、Browser portや既存processを所有・停止しない。公開/installer/deploymentは今回のscope外 |
...
- WPF Release build、`verify-wpf-product.ps1` default aggregate、cross-runtime Favorite/Seen/Recent、40-cycle reload soak。aggregateのcheck数はinventoryから動的に決まり、文書基準`64472dd`ではdefault 41、SkipStress 40、IncludeReloadSoak 42。固定数ではなくlive JSONの0 failureをgateにする。
...
5. 1,000枚×2 folderを同一WPF processで40 cycle切替/Refreshし、metadata/preview/modal遅延、explicit cancel 14、supersede 26、rapid search、Grid/List、preview tab、Modalを混在させた。stale completion 0、preview/modal 40/40、load CTS 121/121、最終catalog/selection/modal/tabは最後folderだけ、shared JSON/source byte不変、Enhancement 0。working setはwarm 207,273,984、forced-GC後final 569,303,040、max 604,921,856 bytes、managedは13,910,656→25,102,320 bytesだった。後半8 cycleは494,305,280〜595,693,568 bytes、平均559,781,888、OLS slope +10,744,783 bytes/cycleと短区間noiseがある一方、warm-up後32 cycleは16増/15減、OLS slope +288,661 bytes/cycle、cycle 27のpeakを以後13 cycle更新せず、単調増加ではない。forced-GC後のworking-set-minus-managed差は193,363,328→544,200,720 bytesで、native allocator/OS working-set保持として性能観測に残す。`verify-wpf-reload-soak.ps1 -Count 1000 -Cycles 40`。
...
- WPF default aggregateのlive check数/全result、cross-runtime 2本、40-cycle reload soak、5,000/20,000件metrics

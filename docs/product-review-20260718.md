...
20. 100,000件/2.9〜3.2MBのshared Favorite/Seenを同じ20操作で3回測定し、modal-next p95 186〜204ms、Favorite p95 233〜251ms、dispatcher最大停止442〜460msを再現した。small controlはそれぞれ5〜6ms、29〜31ms、38〜53msであり、decodeではなくUI-thread full-store persistenceへ局在するP1 REDである。production async writerはfault/close semanticsを急造せず、`tasks/wpf-next-upgrade-20260718/baseline-results.md`へexact gateと実装境界を固定した。
...
- WPF Release build、`verify-wpf-product.ps1 -IncludeReloadSoak`、cross-runtime Favorite/Seen/Recent。`-IncludeReloadSoak`は既定24-cycle soakをaggregate内で実行し、40-cycle以上は長期memory傾向のextended観測として分ける。shared-state latency gate追加後のimplementation `2e62965`では45/45、276,690ms。check数はinventoryから動的に決まり、固定数ではなくlive JSONの0 failureをgateにする。
...
20,000件の最終観測値（このmachine/fixtureの証拠であり製品hard閾値ではない）: fixture 9,002ms、load 11,362ms、final rapid search 348ms、Grid 96/maximum 384、List 22、heartbeat 18、working set 144,769,024→259,657,728 bytes、GC gen0/1/2 = 22/14/3。source 20,000維持、truncate 0、Enhancement 0。初回観測はfixture 5,953ms、load 8,180ms、search 332ms、working set 147,943,424→370,585,600 bytes、GC 25/16/6だった。

...
| --- | --- | --- |
| `GIT-LOCAL-001` | commit | task開始時local main `4c81cca3` を起点に、Modal/filmstrip、WPF zoom/anchor、shared-state latencyを保持し、Browser rainbow枠`2863519`、WPF Modal/status枠`a091ec7`、WPF Filmstrip保存`c3d4ff5`まで同系列で採用した。旧default origin/mainは実装基準にしない |
...
| thumbnail status borders | Favorite黄色inner、Enhanced虹色outer、独立ON/OFF、rainbow/単色、Grid/List同時表示、shared保存保護 | `ADOPT-STATUS-005` |
...
| thumbnail status borders | Browser同一schema、Favorite黄inner、Enhanced虹outer、独立ON/OFF/単色、Freeze済みshared brush、Grid/List O(1) | `ADOPT-STATUS-005`、`verify-wpf-thumbnail-status-borders.ps1`全true |
...
| 3 | Album/collection発掘 | pending discovery | branches/history inventory、product model、shared ownership、migration/conflict |

...
| Album v1 product surface | shared operation API、library/picker、current/outside/missing availability、guarded source session、Album順Gallery/Modal/Filmstrip、removeとsource Recycleの分離、Recycle成功後cleanup、collision-aware `addToAlbum` shortcut | `AlbumContext.tsx`, `AlbumLibrary.tsx`, `AlbumPicker.tsx`, `viewer-albums.spec.ts`, `verify-wpf-album-store.ps1` |
...
| done | Album v1 semantic recovery | implemented at local checkpoint; GitHub promotion frozen | `tasks/album-v1-semantic-recovery-20260720/checkpoint-2.md`、最終provenance |

...
| Album/collection | `implementation in progress` | v1 shared document、revision、operation API、Browser/WPF storeとcross-runtime verifierは実装済み。product UI/source sessionは未実装 | library/picker、current/outside/missing表示、Modal/Filmstrip source、shortcut/focus、Recycle後cleanup接続 |

### 3.4 blocked

現時点でBrowser coreを止める外部blockerは確認していない。Album/collectionは安全なshared-store checkpointまで進み、UI/source integrationは継続中。具体的な外部条件待ちになった時だけ`blocked`へ移す。
...
| Album v1 shared storage core | Browser同一`albums.json`、opaque id、revision、create/update/delete/add/remove/recent/cleanup、create-new lock、latest-disk mutation、atomic publish、unknown-field保持 | `AlbumStore.cs`, `verify-wpf-album-store.ps1`, Browser/WPF interleave revision 2→6→7 |
...
| Album surface | disabled Album button + deferred tooltip、shared store/mutation core | library/picker、Album source navigation、Filmstrip/Modal、availability UI、shortcut/focus未実装 | `AlbumStore.cs`, `verify-wpf-album-store.ps1`, `WPF-SRC-001` |
...
| Album/collection | `implementation in progress` | Browser同一ownership/model/storeは採用済み。surfaceはdisabled placeholderだけ | Browserと同じoperation semanticsでbasic UI/sourceを接続 |

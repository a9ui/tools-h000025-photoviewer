...
`refs/heads/main` は、同実装を含むrepository hardening descendant `4c81cca3efc80363568d0d9af35297ff3285b48c` を指す。
WPF gallery zoom/geometry anchorは、指定baseline `492988ba2390172c3331b72762f5cecd907a3605` から作った
専用branchの実装commit `4a22b61` まで検証済みだが、まだ`refs/heads/main`へは採用していない。
...
| `WPF-ZOOM-CANDIDATE-001` | pending-adoption evidence | `codex/wpf-gallery-zoom-anchor-adoption-20260719` の `4a22b61` はcandidate `b0f9a0e9`を`492988ba`へmechanical cherry-pickせずsemantic mergeした。20〜600/step 20、600=1列、旧40維持・範囲外clamp、canonical full path+viewport offsetをzoom/Sidebar/right panel/window resize/DPI/selection有無で保持する。focused、現行aggregate 53/53 + reload soak 24/24、exact 100,000/100 folders、Favorite/Seen/Recent/Search History cross-runtime各20反復がgreen。通常launcher/runtimeはscope禁止により未実行 |
...
| Capability | Present part | Missing / mismatch | Evidence |
| --- | --- | --- | --- |
| gallery zoom | local mainは40〜600、step 20、thumb-change zoom anchor。候補commitは20〜600、600=1列、migration/virtualization gate済み | `refs/heads/main`採用と通常launcher/runtime確認が未実施 | `WPF-SRC-001`, `WPF-ZOOM-CANDIDATE-001` |
| Sidebar toggle | local mainはVisibility + 240/0。候補commitはcanonical full path/viewport offsetをSidebar/right panel/window resize/DPIで保持 | `refs/heads/main`採用と通常launcher/runtime確認が未実施 | `WPF-SRC-001`, `WPF-ZOOM-CANDIDATE-001` |
...
| Requirement | Status | local main truth | Adoption gate |
| --- | --- | --- | --- |
| 20px〜最大1列 Browser parity | `pending adoption` | local mainは40〜600。候補commit `4a22b61` は20〜600/600=1列、旧40維持・範囲外clamp、Grid/List bounded | local mainへsemantic adoption後、通常launcher/runtime |
| Sidebar開閉アンカー Browser parity | `pending adoption` | local mainは240/0 toggleのみ。候補commitはsame canonical path/offset、Sidebar/right panel/window resize/DPI、selection有無を0px driftで検証 | local mainへsemantic adoption後、通常launcher/runtime |
...
| --- | --- | --- | --- |
| 1 | WPF 20px〜最大1列 + Sidebar reopen anchor parity | pending adoption | source/state、focused、53-check aggregate、exact 100k/100folder、shared stateは候補commitでgreen。残りはcurrent mainへのsemantic adoptionと通常launcher/runtime |
...
| `KNOWN-WPF-002` | WPF parity | Browserで採用した20px/1列とSidebar reopen anchorはlocal main未採用。専用候補commitとgate証拠は存在する | candidateをlocal main implementedへ横流しせず、採用後に通常launcher/runtimeを確認する |
| `KNOWN-GIT-002` | repo | branch作成後に別laneが`refs/heads/main`を`492988ba`から`4c81cca3`へ進め、`docs/current-implementation-truth.md`も変更した | `4a22b61`をblind cherry-pickせず、main側hardening/docsを保持してsemantic adoptionする |

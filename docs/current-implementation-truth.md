...
独立review hardeningはlocal `main` HEAD `f64d98d81e016e43177b1e39738f759742d1215b`
上の未commit worktree deltaであり、最終focused/normal-scale full、runtime/provenance、
public-guard tests、SHAは**PENDING FINAL GATE**である。20k/100k catalog-scaleとaggregate
check-count追跡はユーザーが今回gateから明示除外した。
...
| `in progress` | current local-main worktreeにsourceがあるが、commitまたは最終focused/full/runtime/provenance gateが未完了 |
...
`implemented`へ予定を含めない。20px〜1列、Sidebar/geometry anchor、Favorite source削除確認、shared Search History、Modal filmstrip、表示中asset open/容量表示、一覧Enter、Album v1はBrowser/WPF、拡大画像session回復はBrowserだけを、local main採用とgate後に昇格した。
...
| `GIT-REMOTE-001` | provenance / freeze | milestone再監査でPR #322 headは`8914935f3118f5634d22570be7d207fbc7735340`、default `origin/main=626b7dd5416f3619ae59fc66d47e79acd1a74fd5`。local mainはそのdescendantでtreeが異なるため、#322は**MERGE FROZEN**。このlaneはpush/merge/closeを行わず、最終commit後にlocal/PR/origin/merge-candidate tree proofを再取得する |
...
| `REVIEW-HARDEN-010` | current worktree implementation | Album legacy-empty migration、live-owner lock保護、latest-state Recycle cleanup retry、unknown key-binding chord予約、Browser direct non-shell openを追加。Browser/WPFの縦Filmstrip、Modal context actions、Settings backdrop/scroll/hover、WPF full-window Modal、28/44/28 edge、gallery Ctrl+wheel/middle auto-scroll、一覧Favorite、solid cyan枠、prompt wrapper正規化を実装した |
| `REVIEW-GATE-011` | current focused evidence | Browser full unit 69 files / 635 tests、typecheck、lint、production buildがgreen。WPF Release 0 warning / 0 errorとAlbum/Modal/context/gallery zoom・auto-scroll/Settings/status-border/prompt/Favorite focused verifierがgreen。最終focused/normal-scale full、Browser/WPF runtime/provenance、public-guard tests、SHAは**PENDING FINAL GATE**。20k/100k/aggregate trackingは今回gateからユーザー明示除外 |
| `PUBLIC-READINESS-012` | publication boundary | Browser direct argument-vector openは採用。`dev`/`start`のexplicit loopback bindとmutating API Origin/Host guardはworktree実装中でtests pending。`LICENSE`未決、full Codex Security scan未実行のためpublic readinessは**NO-GO**、private/local・deployment禁止を維持 |
...
| `BR-ALBUM-TEST-006` | Album v1 Browser acceptance | current local treeの`pnpm test:unit`は68 files / 614 tests PASS、3 files / 3 tests skip。typecheck、ESLint、Next.js 16.2.10 production buildがgreen。isolated `127.0.0.1:3131` Album Playwright 1/1はcreate/add、current/outside/missing、Album順Modal/Filmstrip、membership removeとsource Recycleの分離、catalog復帰、console error 0を確認し、port 3000を使用していない |
| `BR-ALBUM-RUN-007` | Album v1 normal-launcher acceptance | standard `prod_launcher.js --port 3132`でimplementation SHA `5f24725`、`sourceDirty=false`、loopback-only、Album Playwright 1/1を確認。共有`.next`更新で旧port 3000 asset 2件が500になったため、正規所有chainをstandard launcherでfinal buildへ復旧し、exact revision、clean、root 200、参照static asset 9/9 HTTP 200を確認。再発防止は`docs/operations-log.md`へ固定 |
| `BR-RUN-001` | normal launcher adoption | Album closeoutで通常WPF Release targetは`current / provenance-match`。通常Browser `127.0.0.1:3000`もstandard launcher ownership、closeout HEAD、`sourceDirty=false`、loopback-only、root 200、参照asset全200へ更新済み。build ID/PIDはvolatileで正本値にせず、`verify-browser-runtime.ps1 -ExpectedRevision`とasset probeでlive判定する |
| `BR-GIT-005` | PR publication boundary | PR #322は**MERGE FROZEN**。recorded remote head `8914935`はlocal hardening worktreeより古く、このlaneはpush/merge/closeを行わない。final candidate化にはexact final SHAのtree/provenance proofとユーザー明示承認が必要 |
...
| `WPF-ALBUM-TEST-004` | Album v1 WPF/current-main acceptance | `verify-wpf-album-store.ps1`は5 files / 24 tests、Release 0/0、WPF UI source/order/availability/shortcut、Browser/WPF barrier 16+16同時writer revision 32/lost 0/residue 0がgreen。最終`verify-wpf-product.ps1 -IncludeReloadSoak`は56/56、311,665ms、reload 24/24。exact 100,000 catalogはsilent truncate 0、Grid/List 15/9、warm hit 100,000/miss 0、anchor drift 0、最大unresponsive 260ms/750msでcleanup成功 |
...
| `STATE-001` | preservation | primary worktreeのtracked `next-env.d.ts`は今回のAlbum/UI実装と無関係なgenerated dirty changeとして除外し、stage/resetしない。Claude worktreeのuntracked `local-native/`と別worktreeのuntracked screenshotも変更・clean・削除しない |
...
| virtualized Modal filmstrip | current追従、direct click、viewport+overscanだけrealize、T/toolbar保存開閉、Arrow navigation、Delete隣接1回。bounded source/chrome基盤はimplemented。縦長左railと左端transient railへの変更はcurrent worktreeで実装済み、最終runtime待ち | `ADOPT-002`, `ADOPT-MODAL-006`, `REVIEW-HARDEN-010`, `REVIEW-GATE-011` |
...
| thumbnail status borders | Favorite黄色inner、Enhanced別色outer、独立ON/OFF、Grid/List同時表示、dirty preference単位latest-disk merge、空patch拒否、shared保存保護。current worktreeはrainbow presentationを廃止しlegacy `rainbow`をsolid cyanへ正規化 | `ADOPT-STATUS-005`, `ADOPT-SETTINGS-007`, `REVIEW-HARDEN-010` |
...
| Album v1 product surface | shared operation API、library/picker、current/outside/missing availability、guarded source session、Album順Gallery/Modal/Filmstrip、removeとsource Recycleの分離。current worktreeはRecycle成功後cleanupのpending表示/latest-state retryとunknown binding chordを含むcollision-aware migrationを追加 | `AlbumContext.tsx`, `AlbumLibrary.tsx`, `AlbumPicker.tsx`, `REVIEW-HARDEN-010`, `verify-wpf-album-store.ps1` |

### 3.2 review hardening in progress
...
| Browser focused image scheduling | `PV_THUMB_CONCURRENCY`をeffective 2〜16へclampし、background thumbnail warmupによる全slot占有を防ぐ。queued directはwarmupより先に開始するが、開始済みdirect/Sharp jobはpreemptしない。有限のdirect負荷が収束した後にwarmup queueをdrainする | exact local commit、consolidated GitHub checkpoint |
...
| image context menu | Modal product menuはcurrent worktreeに実装 | Explorer/path/image clipboard actionsと最終runtime evidenceは未完 | `REVIEW-HARDEN-010`, `ImageModal.test.tsx` |
...
現時点でBrowser coreを止める外部blockerは確認していない。Album v1のBrowser product surfaceとWPF basic surfaceは実装・runtime検証済み。具体的な外部条件待ちになった時だけ`blocked`へ移す。
...
| Album v1 shared product | Browser同一`albums.json`、opaque id、revision、create/update/delete/add/remove/recent/cleanup、create-new lock、latest-disk mutation、atomic publish、unknown-field保持。library/picker、current-only Album source、Album順filter/Modal、outside/missing明示、source Recycle成功後cleanup、collision-aware shortcut | `AlbumStore.cs`, `AlbumLibraryWindow.cs`, `verify-wpf-album-store.ps1`, Browser/WPF barrier同時writer、WPF UI smoke |
...
| Modal manual/transient UI + Filmstrip | manual visible固定、hidden cursor + 900ms transient、button-focus shortcut、hidden-state維持、ViewerState保存。current worktreeはfull-window Uniform image area、28/44/28 edge、single zoom indicator、左の縦Filmstripへ更新 | `ADOPT-MODAL-006`, `REVIEW-HARDEN-010`, focused `verify-wpf-modal-interaction.ps1` |
...
| thumbnail status borders | Browser同一schema、Favorite黄inner、Enhanced solid cyan outer、独立ON/OFF/単色、Grid/List O(1)、dirty preference単位latest-disk merge。legacy rainbowはcyanへmigration | `ADOPT-STATUS-005`, `ADOPT-SETTINGS-007`, `REVIEW-HARDEN-010`, focused status-border verifier |

### 4.2 review hardening in progress

| Capability | Current worktree result | Remaining gate |
| --- | --- | --- |
| shell / Settings / hover | Landing caption controls、sidebar下端Settings、single settings scroll/backdrop close、enabled-only hover/focus、dark tooltip | final focused/full normal-scale、normal launcher/runtime |
| gallery input / cards | active Grid surface/scrollbar Ctrl+wheel、non-gallery isolation、List row非拡縮、middle-click auto-scroll、Grid/List direct Favorite、small-card geometry、neutral background | final focused/full normal-scale、normal runtime |
| Modal action surface | full-window Uniform image、縦Filmstrip、28/44/28 edge、image/empty chrome toggle、Enhanced default/Original toggle、functional top buttons、product context menu、single zoom indicator | final focused/full normal-scale、normal runtime/provenance |
| prompt display | emphasis-only wrapperを除去しBrowser同様のplain tag表示 | final focused/full normal-scale |
...
| Album outside member open | libraryでoutside/missing件数とunavailableを明示 | current catalog外の任意pathはWPF v1で開かない。folderをcatalogへ追加してcurrentにする必要がある | `AlbumLibraryWindow.cs`, `verify-wpf-album-store.ps1` |
| image context menu | Modal product menuはcurrent worktreeに実装 | path/image clipboard copyと最終runtime evidenceは未完 | `REVIEW-HARDEN-010`, focused Modal context action smoke |
...
| --- | --- | --- | --- |
| 1 | finish image context menu + Ctrl+C image copy | context action menus in progress; clipboard pending | final runtime、path/image clipboard format/error、a11y |
| in progress | Album v1 review hardening | focused green; final gate pending; GitHub promotion frozen | `tasks/album-v1-semantic-recovery-20260720/checkpoint-3-review-hardening.md`、focused/full normal-scale、runtime/public guard、final provenance |
...
| `KNOWN-BR-005` | Browser runtime | review-hardening deltaのisolated Browser runtimeは未実行。tracked `next-env.d.ts`はunrelated generated dirty | clean扱いせずcheckpointから除外。port 3001+でfinal sourceを検証する |
...
| `KNOWN-GIT-001` | repo | PR #322 recorded head `8914935`はlocal mainより古く、current hardeningは未commit | **MERGE FROZEN**。古いorigin/mainをbaselineにせず、final commit後もpush/merge/closeはユーザー明示承認まで行わない |
| `KNOWN-REVIEW-001` | common | review hardeningはfocused greenだがfinal focused/full normal-scale、Browser/WPF runtime/provenance、public-guard tests、final SHAが未確定 | `PENDING FINAL GATE`。20k/100kとaggregate check trackingはユーザー明示除外。過去checkpointのgreenはcurrent deltaへ横流しせず、今回rerunも要求しない |
| `KNOWN-PUBLIC-002` | repo/publication | direct argv openは採用済み。loopback package scriptsとmutating API Origin/Host guardはtests pending。`LICENSE`未決、full Codex Security scan未実行 | public readiness **NO-GO**。private/local・deployment禁止を維持し、scanは外部相談禁止の本lane外で別途authorizeする |

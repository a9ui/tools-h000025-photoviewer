...
| --- | --- | --- |
| `GIT-LOCAL-001` | commit | 通常rootの実装revisionは `792716f9863dd145e028648405c29c340f7a4336`。`refs/heads/main` はそのdocs-only descendant。実装は旧default origin/mainではなく、検証済みlocal main上で継続した |
| `GIT-REMOTE-001` | provenance | default `origin/main=626b7dd5416f3619ae59fc66d47e79acd1a74fd5` は古いまま。Draft PR [#322](https://github.com/a9ui/tools-h000025-photoviewer/pull/322) のremote headも `557cfb8e387aaf8ddc12be7b6408cdb8573ef4a2` で、local mainのfilmstrip/Search History採用commitをまだ保持していない。`git push origin main:codex/wpf-thumbnail-performance-20260719` は2026-07-19にexternal-write approval policyでprocess作成前に拒否された。SQLite improvement item #45をqueuedとし、push成功まではPR/default originを正本にしない |
...
| `KNOWN-GIT-001` | repo | default origin/mainとDraft PR #322 headはlocal mainより古い | local commitを正本として保持し、approved external-write sessionでPR branchへpushする。成功までは「GitHub退避済み」と主張しない。新規作業を古いorigin/mainから開始せず、deploymentしない |

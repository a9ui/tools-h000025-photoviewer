...
Modal/filmstrip layout `9d8acb0335392687a801504ecfcd09dda6b50be6`、Enhancement修正を保持したdescendantである。
...
`implemented`へ予定を含めない。20px〜1列、Sidebar/geometry anchor、Favorite source削除確認、shared Search HistoryはBrowser/WPF、拡大画像session回復とModal filmstripはBrowserだけを、local main採用とgate後に昇格した。

## 1. Evidence register

| ID | 種別 | 2026-07-19に確認した証拠 |
| --- | --- | --- |
| `GIT-LOCAL-001` | commit | task開始時local main `4c81cca3` を起点に、Modal/filmstrip `9d8acb0`を保持してWPF zoom/anchorを`e371b48`へ採用し、shared-state latencyを`5ae1e00`で改善した。current mainは以後のBrowser修正も含む同系列で継続する。旧default origin/mainは実装基準にしない |
...
| virtualized Modal filmstrip | current追従、direct click、viewport+overscanだけrealize、T/toolbar保存開閉、Arrow navigation、Delete隣接1回。専用下段を確保して画像viewportを縮め、画像へ重ねない。3秒auto-hide/re-show、上側zoom indicator | `ADOPT-002`, `ADOPT-BR-003`, `BR-TEST-001`, `BR-E2E-002` |
...
| --- | --- | --- | --- |
| 1 | displayed Original/Enhanced Enter + WPF一覧Enter Modal | pending | input/overlay isolation、focus return、path guard、both display state |
| 2 | image context menu + Ctrl+C image copy | pending design/implementation | action model、selection、clipboard format/error、a11y |
| 3 | Enhancement枠色設定 | pending design/implementation | schema ownership、validation、persistence、visual/a11y |
| 4 | Album/collection発掘 | pending discovery | branches/history inventory、product model、shared ownership、migration/conflict |

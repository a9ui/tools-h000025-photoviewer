...
a native migration target. Browser modal edge-click ratio stays inside the raw
`browser_pvu_view` mirror; native does not create `modal_edge_ratio`,
...
queue panel visibility stays inside the raw `browser_pvu_view` mirror; native
does not create `enhance_queue_open`, `enhanceQueueOpen`,

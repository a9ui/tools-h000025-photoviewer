...
native migration target. Browser `sidebarOpen` controls whether the browser
left sidebar renders, while native currently has no accepted persisted
left-sidebar collapse contract. Explicit exports keep `sidebarOpen` inside the
raw `browser_pvu_view` mirror; native does not create `sidebar_open`,
`sidebarOpen`, `sidebar_visible`, `left_sidebar_open`, or
`left_panel_visible` settings, and `pvu_state_migration_count` stays at the Row
11 count of 11.

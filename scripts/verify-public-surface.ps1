...
try {
    ...
    $patterns = [ordered]@{
        ...
        'private-tools-root' = '(?i)Desktop[/\\]+Tools|<TOOLS_ROOT>'
    ...
    $workflow = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $Root $workflowPath)
    if ($workflow -notmatch '(?ms)^permissions:\s*\r?\n\s+contents:\s*read\s*$') {
        Add-Finding -Rule 'workflow-permissions' -Path $workflowPath -Line 1 `
            -Message 'The verify workflow must declare top-level contents: read permissions.'
    }
    if ($workflow -match '(?m)^\s*pull_request_target\s*:') {
        Add-Finding -Rule 'pull-request-target' -Path $workflowPath -Line 1 `
    ...
    foreach ($line in $usesLines) {
        ...
        if ($reference -notmatch '@[0-9a-fA-F]{40}$') {
            Add-Finding -Rule 'unpinned-action' -Path $workflowPath -Line 1 `
                -Message "GitHub Action is not pinned to a full commit SHA: $reference"
        }
    }
    if ($workflow -notmatch '(?ms)uses:\s*actions/checkout@[0-9a-fA-F]{40}.*?persist-credentials:\s*false') {
        Add-Finding -Rule 'checkout-credentials' -Path $workflowPath -Line 1 `
    ...
    if ($ExpectedRepository) {
        ...
        $oldSlugMatches = @(& git grep -nI 'tools-h000025-photoviewer' -- ':!docs/publication-runbook.md' ':!project.toml' 2>$null)
